	...
	if ($Joueurs[$Membre]->Pseudonyme==$Joueur->Pseudonyme){
		...
		if (($AncienMotDePasse)&&($MotDePasse1)&&($MotDePasse2)){
			if ($MotDePasse1==$MotDePasse2){
				if (mySql_fetch_object(mySql_query("SELECT * FROM NPVB_Joueurs WHERE (Pseudonyme='".$Joueur->Pseudonyme."' AND Password='".old_password_hash($AncienMotDePasse)."')", $sdblink))){
					$MAJMotDePasse = ", Password='".old_password_hash($MotDePasse1)."'";
