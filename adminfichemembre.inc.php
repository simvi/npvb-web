...
if ((($Mode=="Modif")||($Mode=="Nouveau")) && !isset($_POST['GenererLienReset'])) {
	...
	if ($Mode=="Nouveau") {
		if (!$MotDePasse){
			$ErreurDonnees["Pseudonyme"] .= "Le mot de passe est obligatoire<br/>pour la création d'un compte";
	...
	}else if (preg_match("/[^a-zA-Z0-9_ àèéùâêîôûëïüÿçÀÈÉÙÂÊÎÔÛËÏÜŸÇ-]/u", $Prenom)){$ErreurDonnees["Civillite"] .= "Le format du Prénom est incorrect<br/>";
	}
	...
	if (($Telephone1)&&(!preg_match("/^[0-9]{10}$/", $Telephone1))){$ErreurDonnees["Contact"] .= "Le premier numéro de téléphone est incorrect<br/>";}
...
?>
				...
				<fieldset>
					...
					<table>
						...
						<tr>
							<td class="Colonne1">Num licence</td>
