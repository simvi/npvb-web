        ...
        foreach ($Evenements[$Jour][$HeureKey] as $Key=>$Event){
            ...
            if (($estDerouleetaitPresent=="o")&&(!$ErreurDonnees)){
                ...
                if (($Event->Libelle<>"ASSO")&&($Event->Libelle<>"SEANCE")){
                    for ($Set=0; $Set<=5; $Set++){
                        eval("if (ereg(\"[0-9]{1".(($Set==0)?"":",2")."}\", $"."Set".(($Set==0)?"s":$Set)."Equipe".$Event->Libelle.$Event->DateHeure.")) { $"."Resultat".$Event->Libelle.$Event->DateHeure." .= ".(($Set==0)?"$"."Set".(($Set==0)?"s":$Set)."Equipe".$Event->Libelle.$Event->DateHeure:"($"."Set".(($Set==0)?"s":$Set)."Equipe".$Event->Libelle.$Event->DateHeure." >= 10)?$"."Set".(($Set==0)?"s":$Set)."Equipe".$Event->Libelle.$Event->DateHeure.":\"0\".(int)$"."Set".(($Set==0)?"s":$Set)."Equipe".$Event->Libelle.$Event->DateHeure)."; }else{ $"."ErreurDonnees[\"Enregistrement\"] .= \"Veillez vérifier le résultat<br/>\"; }");
                        eval("if (ereg(\"[0-9]{1".(($Set==0)?"":",2")."}\", $"."Set".(($Set==0)?"s":$Set)."Adversaire".$Event->Libelle.$Event->DateHeure.")) { $"."Resultat".$Event->Libelle.$Event->DateHeure." .= ".(($Set==0)?"$"."Set".(($Set==0)?"s":$Set)."Adversaire".$Event->Libelle.$Event->DateHeure:"($"."Set".(($Set==0)?"s":$Set)."Adversaire".$Event->Libelle.$Event->DateHeure." >= 10)?$"."Set".(($Set==0)?"s":$Set)."Adversaire".$Event->Libelle.$Event->DateHeure.":\"0\".(int)$"."Set".(($Set==0)?"s":$Set)."Adversaire".$Event->Libelle.$Event->DateHeure)."; }else{ $"."ErreurDonnees[\"Enregistrement\"] .= \"Veillez vérifier le résultat<br/>\"; }");
                    }
                }
