...
if (!peutAccederPage($Joueur, 'adminevenements')){ require("accueil.inc.php"); return;}
			...
			while (!feof($FichierImportEvenements)){
				for ($index=0; $index<count($EvenementsImportesTMP); $index++){
					if ($Ligne==-1){
						...
						$EvenementsImportes[$Ligne][$TitresAtributs[$index]] = trim(str_replace("�", "-", $EvenementsImportesTMP[$index]));
					}
				}
				if ($Ligne>=0){
					...
					if (!preg_match("/^[0-9]{14}$/", $DateHeure)) $ErreurDonnees["Donnees"] .= "La format de la date ou de l'heure à la ligne ".($Ligne+1)." est invalide<br/>".$DateHeure;
...
//******************
//** FinRécupère et teste les infos de tout le fichier
//******************


//******************
//** S'il n'y a pas d'erreur dans le fichier, enregistrer les événements
//******************
